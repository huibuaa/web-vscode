...
export function createPromptGenerateComment(language: string, functionCode: string): string {
    return `${l10n.t('Please add comments to the following {0} code. Return only the full commented code in a single code block without extra explanation', language)}\n\`\`\`${language}\n${functionCode}\n\`\`\`\n`;
}

export function createPromptUseCaseAnalysis(language: string, functionCode: string): string {
    return `${l10n.t('Please analyze use cases for the following {0} code, including typical inputs, outputs, edge cases, and exception scenarios', language)}\n\`\`\`${language}\n${functionCode}\n\`\`\`\n`;
}
...
export function createPromptFixBugs(language: string, functionCode: string): string {
    return `${l10n.t('Please check and fix bugs in the following {0} code', language)}\n\`\`\`${language}\n${functionCode}\n\`\`\`\n`;
}
